...
NDiplomacy = {
	...
	BASE_TRUCE_PERIOD = 360,						--WAS 30 Base truce period in days.
	...
	VOLUNTEERS_PER_COUNTRY_ARMY = 0.03,				--WAS 0.05 Each army unit owned by the source country contributes this amount of volunteers to the limit.
	VOLUNTEERS_RETURN_EQUIPMENT = 0.70,				--WAS 0.95 Returning volunteers keep this much equipment
	VOLUNTEERS_TRANSFER_SPEED = 30,					--WAS 14 days to transfer a unit to another nation
	...
	TENSION_SIZE_FACTOR = 1,						-- action tension values are multiplied by this value
...
NCountry = {
	...
	POPULATION_YEARLY_GROWTH_BASE = 0.035,			--WAS 0.015 basic population growth per year, used for monthly manpower gain
	...
	DEFAULT_NATIONAL_UNITY = 0.6,					--WAS 0.7 Default national unity if not scripted otherwise.
	MAX_NATIONAL_UNITY = 0.99,                      --WAS 0.95
...
NProduction = {
	...
	BASE_FACTORY_EFFICIENCY_GAIN = 0.11,		--WAS 1 -- Base efficiency gain for factories per unit produced in %.
	...
	ANNEX_CONVOYS_RATIO = 0.30,			--WAS 0.15 How many convoys will be transferred on annexation
	...
	INFRA_MAX_CONSTRUCTION_COST_EFFECT = 0.7, 		--WAS 1 Building in a state with higher infrastructure will reduce the cost of shared buildings.
...
NBuildings = {
	...
	SUPPLY_PORT_LEVEL_THROUGHPUT = 4,   --WAS 3.0 supply throughput per level of naval base
	INFRA_TO_SUPPLY = 2,				-- 
...
NMilitary = {
	...
	CORPS_COMMANDER_CAP = 0,			--WAS 24 how many a corps commander is limited to.
	...
	COMBAT_MINIMUM_TIME = 6,			--WAS 4 Shortest time possible for a combat in hours
	...
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.05,        -- global damage modifier
	...
	ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.15,		   --WAS 0.1 Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	ATTRITION_EQUIPMENT_PER_TYPE_LOSS_CHANCE = 0.12, --was 0.1 Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	DIG_IN_FACTOR = 0.05,						   --WAS 0.02 bonus factor for each dug-in level
	LEADER_SKILL_FACTOR = 0.07,                     --WAS 0.05 bonus per skill level of leader
	BASE_LEADER_TRAIT_GAIN_XP = 0.75,			   --WAS 1 Base xp gain for traits per hour.
	...
	LEADER_EXPERIENCE_SCALE = 0.66,                 --WAS 1
	...
	EQUIPMENT_COMBAT_LOSS_FACTOR = 0.72,	 	   --WAS 0.7 % of equipment lost to strength ratio in combat, so some % is returned if below 1
	SUPPLY_USE_FACTOR_MOVING = 1.85,                --WAS 1.5 supply use when moving/fighting vs inactive
	...
	NON_CORE_SUPPLY_SPEED = -0.6,				   --WAS -0.5 we are not running on our own VP supply so need to steal stuff along the way
	OUT_OF_SUPPLY_MORALE = -0.4,                   --WAS -0.3 max org regain reduction from supply
	TRAINING_ATTRITION = 0.03,		  			   --WAS 0.06 amount of extra attrition from being in training
	...
	ORG_LOSS_FACTOR_ON_CONQUER = 0.15,              --WAS 0.2 percentage of (max) org loss on takign enemy province
	...
	PLAN_SPREAD_ATTACK_WEIGHT = 5.0,				--WAS 13.0 The higher the value, the less it should crowd provinces with multiple attacks.
	PLAN_SUPPLY_FACTOR = 0.15,						--WAS 0.1 Factor applied to available supply on a front location when determining priority
	...
	PLAN_PROVINCE_HIGH_VP_IMPORTANCE_AREA = 100.0,   --WAS 50.0 Used when calculating the calue of defense area in the battle plan system
	...
	PLAN_MAX_PROGRESS_TO_JOIN = 0.05,				--WAS 0.5 If Lower progress than this, probably needs support
	...
	MIN_DIVISION_DEPLOYMENT_TRAINING = 0.01,			--WAS 0.2 Min level of division training
...
NAir = {
	...
	FIELD_EXPERIENCE_MAX_PER_DAY = 3,					--WAS 2 Most xp you can gain per day
...
NNavy = {
	...
	COMBAT_TORPEDO_CRITICAL_DAMAGE_MULT = 5.0,						--WAS 2 multiplier to damage when got critical hit from torpedo. (Critical hits are devastating as usualy torpedo_attack are pretty high base values).
...
NTrade = {
	DISTANCE_TRADE_FACTOR = -0.0002,			--WAS -0.02 Trade factor is modified by distance times this
	...
	MAX_MONTH_TRADE_FACTOR = 150,			--WAS 50 This is the maximum bonus that can be gained from time
...
NAI = {
	...
	CALL_ALLY_WT_LIMIT = 0.95,                           --WAS 0.8 fascists are hesitant to call allies if tension is lower than this
	CALL_ALLY_FASCIST_DESIRE_WT = -100,                  --WAS -60 less desire if we are below the tension limit
	...
	MIN_SUPPLY_USE_SANITY_CAP = 500,					--WAS 100 Ignore supply cap if below this value when deciding on how many divisions to produce.
	MAX_SUPPLY_DIVISOR = 25.0,							--WAS 1.75 To make sure the AI does not overdeploy divisions. Higher number means more supply per unit.
	...
	DOCKYARDS_PER_NAVAL_DESIRE_EFFECT = -10.0,			--WAS -20.0 Effects how much AI wants to build dockyards based on how navally focused they are in general. Recommended range -100.0 to 100.0.
},
